        ...
        if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
          ...
          if DoesEntityExist(vehicle) then
            ...
            Citizen.CreateThread(function()
              ...
              exports['mythic_notify']:DoHudText('success', _U('vehicle_unlocked'))
        ...
        if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
          ...
          if DoesEntityExist(vehicle) then
            ...
            Citizen.CreateThread(function()
              ...
              ClearPedTasksImmediately(playerPed)
              exports['mythic_notify']:DoHudText('success', _U('vehicle_repaired'))
      ...
      if data.current.value == 'dep_vehicle' then
        ...
        if isVehicleTow then
          ...
          Citizen.CreateThread(function()
            while true do
              ...
              if IsEntityUpsidedown(GetVehiclePedIsIn(PlayerPedId(), true)) and isVehicleTow or roll > 70.0 or roll < -70.0 then
                ...
                exports['mythic_notify']:DoHudText('error', 'Cables holding the vehicle on broke!')
              end        
            end
          end)
          ...
          if currentlyTowedVehicle == nil then
            if targetVehicle ~= 0 then
              ...
              if distanceBetweenVehicles > 12.0 then
                exports['mythic_notify']:DoHudText('error', "Your cables can't reach. Move your truck closer")
              else
                ...
                if not ((not allowTowingBoats and IsThisModelABoat(targetModelHash)) or (not allowTowingHelicopters and IsThisModelAHeli(targetModelHash)) or (not allowTowingPlanes and IsThisModelAPlane(targetModelHash)) or (not allowTowingTrains and IsThisModelATrain(targetModelHash)) or (not allowTowingTrailers and isTargetVehicleATrailer(targetModelHash))) then 
                  if not IsPedInAnyVehicle(playerped, true) then
                    if vehicle ~= targetVehicle and IsVehicleStopped(vehicle) then
                      ...
                      exports['mythic_notify']:DoHudText('inform', _U('vehicle_success_attached'))
                    else
                      exports['mythic_notify']:DoHudText('error', "There is currently no vehicle on the flatbed.")
                    end
                  else
                    exports['mythic_notify']:DoHudText('error', "You need to be outside of your vehicle to load or unload vehicles.")
                  end
                else
                  exports['mythic_notify']:DoHudText('error', "You cannot tow this vehicle")
                end
              end
            else
              exports['mythic_notify']:DoHudText('error', "No towable vehicle detected.")
            end
          elseif IsVehicleStopped(vehicle) then
            ...
            currentlyTowedVehicle = nil
            exports['mythic_notify']:DoHudText('inform', _U('veh_det_succ'))
          end
        else
          exports['mythic_notify']:DoHudText('error', "You need a flatbed")
        end
      end
