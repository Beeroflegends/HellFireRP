...
AddEventHandler("esx_inventoryhud:tradePlayerItem", function(from, target, type, itemName, itemCount)
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'No puedes llevar mas'})
                end
            end
...
ESX.RegisterServerCallback("suku:getShopItems", function(source, cb, shoptype)
	...
	for i=1, #itemResult, 1 do
		...
		if shoptype == "mafiashop" then
			local weapons = Config.Shops.MafiaShop.Weapons
			for _, v in pairs(Config.Shops.MafiaShop.Weapons) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_weapon",
						name = itemInformation[itemResult[i].name].name,
			...
			local ammo = Config.Shops.MafiaShop.Ammo
			for _,v in pairs(Config.Shops.MafiaShop.Ammo) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_ammo",
						name = itemInformation[itemResult[i].name].name,
			...
			for _, v in pairs(Config.Shops.MafiaShop.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
...
AddEventHandler('suku:buyLicense', function ()
	...
	if bank.money >= Config.LicensePrice then
