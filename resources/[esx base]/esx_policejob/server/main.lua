...
TriggerEvent('es:addCommand', 'malikontrol', function(source, args, user)
    local Source = source
    local number = tonumber(args[1])
    local xPlayer = ESX.GetPlayerFromId(Source)
	local targetPlayer = ESX.GetPlayerFromId(number)
	local bankaParasi = targetPlayer.getBank(number)
	local ustundekiPara = targetPlayer.getMoney(number)
	local ismiNe = targetPlayer.getName(number)
	
    if args[1] ~= nil then 
		if xPlayer.job.name == 'police' and type(number) == "number" then
        	
			TriggerClientEvent("pNotify:SendNotification", source,{
                    text = (" " ..ismiNe .. " Kişisinin nakit " .. ustundekiPara .. "$ bankasında ise " .. bankaParasi .. "$ var."),
                    type = "info",
					theme = "metroui",
                    timeout = 5000,
                    layout = "topRight",
