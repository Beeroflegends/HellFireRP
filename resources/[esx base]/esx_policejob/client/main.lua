...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		elements = {
			...
			{label = "Vehicle Tracker",			value = 'tracker'},
			{label = "Jail Menu",               value = 'jail_menu'}
		}
	}, function(data, menu)

        if data.current.value == 'jail_menu' then
            TriggerEvent("esx-qalle-carcel:openJailMenu")
        end
		
		if data.current.value == 'tracker' then
            TriggerEvent("lspd_tracker:open")
        end
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = "Hardcuff", value = 'handcuffhard'},
				{label = "Remove Hardcuff", value = 'handcuffunhard'},
		...
		--		{label = "Kamu Görevi",	value = 'communityservice'}
			}
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuffhard' then
					...
					elseif action == 'handcuffunhard' then
					local target, distance = ESX.Game.GetClosestPlayer()
  					 playerheading = GetEntityHeading(GetPlayerPed(-1))
   					 playerlocation = GetEntityForwardVector(PlayerPedId())
   					 playerCoords = GetEntityCoords(GetPlayerPed(-1))
...
function SendDistressSignal()
    ...
    TriggerServerEvent('esx_addons_xenknight:startCall', 'ambulance', _U('distress_message'), PlayerCoords, {
