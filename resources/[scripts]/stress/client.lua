    ...
    TriggerEvent('esx_status:getStatus','stress',function(status)
      ...
      if status.val >= (self.GetSickAt * 10000) then
        ...
        if not self.IsSick then
          self.IsSick = true
          exports['mythic_notify']:DoHudText('inform', 'Te estas estresando', 2500, { ['background-color'] = '#B04361', ['color'] = '#ffffff' })
        else
          ...
          if status.val >= (self.ExtremelySickAt * 10000) then
            ...
            if not self.ExtremelySick then
              ...
              exports['mythic_notify']:DoHudText('inform', 'Tanto estres te pone mal', 2500, { ['background-color'] = '#B04361', ['color'] = '#ffffff' })
          ...
          elseif self.ExtremelySick then 
            ...
            exports['mythic_notify']:DoHudText('inform', 'Te sientes mejor', 2500, { ['background-color'] = '#B04361', ['color'] = '#ffffff' })
          end
        end
      elseif status.val <= (self.GetSickAt * 10000) then
        if self.IsSick then
          self.IsSick = false
          exports['mythic_notify']:DoHudText('inform', 'Te sientes mas relajado', 2500, { ['background-color'] = '#B04361', ['color'] = '#ffffff' })
...
function MFS.DoEat(source,getHigh)
  local self = MFS
  if not source then return; end
  local adder = 0
  if getHigh then adder = self.JointRelief
...
NewEvent(true,MFS.DoEat,'MF_Stress:Eat')
