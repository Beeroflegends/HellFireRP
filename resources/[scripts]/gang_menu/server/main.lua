...
--[[	if sourceXPlayer.job.name ~= 'police' or sourceXPlayer.job.name ~= 'ambulance' or sourceXPlayer.job.name ~= 'mechanic' then
		print(('esx_ballas: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end ]]--
...
RegisterServerEvent('esx_ballas:requestarrest')
AddEventHandler('esx_ballas:requestarrest', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_policia:handcuff', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_ballas:doarrested', _source)
end)
...
AddEventHandler('esx_ballas:drag', function(target)
	...
	TriggerClientEvent('esx_ballas:drag', target, source)
...
AddEventHandler('esx_ballas:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'police' then
		...
		print(('esx_ballas: %s attempted to drag out from vehicle (not cop)!'):format(xPlayer.identifier))
	end
end) ]]--
...
RegisterServerEvent('esx_ballas:getStockEItem')
AddEventHandler('esx_ballas:getStockEItem', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local sourceItem = xPlayer.getInventoryItem(itemName)
...
RegisterServerEvent('esx_ballas:putStockEItems')
AddEventHandler('esx_ballas:putStockEItems', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
	local sourceItem = xPlayer.getInventoryItem(itemName)
...
ESX.RegisterServerCallback('esx_ballas:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
ESX.RegisterServerCallback('esx_ballas:getArmoryEWeapons', function(source, cb)
...
ESX.RegisterServerCallback('esx_ballas:addArmoryEWeapon', function(source, cb, weaponName, removeWeapon)
...
ESX.RegisterServerCallback('esx_ballas:removeArmoryEWeapon', function(source, cb, weaponName)
...
ESX.RegisterServerCallback('esx_ballas:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_ballas: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	end
	...
	elseif type == 2 then
		...
		if component then
			...
			print(('esx_ballas: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
			cb(false)
		end
	end
end) 
...
--[[
ESX.RegisterServerCallback('esx_ballas:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_ballas: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	end
...
ESX.RegisterServerCallback('esx_ballas:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_ballas: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
ESX.RegisterServerCallback('esx_ballas:getStockEItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police_evidence', function(inventory)
		cb(inventory.items)
	end)
end)
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'police' then
			Citizen.Wait(5000)
			TriggerClientEvent('esx_ballas:updateBlip', -1)
		end
	end	
end)

RegisterServerEvent('esx_ballas:spawned')
AddEventHandler('esx_ballas:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_ballas:forceBlip')
AddEventHandler('esx_ballas:forceBlip', function()
	TriggerClientEvent('esx_ballas:updateBlip', -1)
end)
...
RegisterServerEvent('esx_ballas:message')
AddEventHandler('esx_ballas:message', function(target, msg)
