...
AddEventHandler('esx_mechanicjob:onFixkit', function()
	...
	-- Animacion
				...
				ClearPedTasks(ped)
				TaskPlayAnim(ped, animDict, anim, 3.0, 1.0, -1, flags, 1, 0, 0, 0)
				exports['progressBars']:startUI(15000,"Intentando arreglar")
				Citizen.Wait(15000)
				StopAnimTask(ped, animDict, anim, 1.0)
				ClearPedTasks(ped)
				RemoveAnimDict(animDict)
				SetVehicleUndriveable(vehicle,false)
				SetVehicleEngineHealth(vehicle, cfg.cascadingFailureThreshold + 100)
				SetVehiclePetrolTankHealth(vehicle, 750.0)
				healthEngineLast=cfg.cascadingFailureThreshold +100
				healthPetrolTankLast=750.0
					SetVehicleEngineOn(vehicle, true, false )
				SetVehicleOilLevel(vehicle,(GetVehicleOilLevel(vehicle)/3)-0.5)
				exports['mythic_notify']:DoHudText('success', 'Ve al mecanico!')
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				exports['mythic_notify']:DoHudText('error', 'El daño es demasiado, llevalo al mecanico')
			end
		else
		...
		exports['mythic_notify']:DoHudText('error', 'Debes estar arriba del vehiculo')
	end
end) ]]--
