    ...
    while true do
        ...
        if shouldDraw or forceDraw then
            ...
            for k, v in pairs(nearbyPlayers) do
                ...
                if  GetPlayerPed( v.id ) ~= GetPlayerPed( -1 ) then
                    if NetworkIsPlayerTalking(v.id) then
                        Draw3DText(x, y, z + 1.1, v.playerId, 5,255,88)
                    else
                        Draw3DText(x, y, z + 1.1, v.playerId, 255,255,255)
                    end
                end
...
function GetNeareastPlayers()
    ...
    for i = 1, #players, 1 do
        found_players = true
        table.insert(players_clean, { id = players[i], playerName = GetPlayerName(players[i]), playerId = GetPlayerServerId(players[i]), coords = GetEntityCoords(GetPlayerPed(players[i])) })
    end
