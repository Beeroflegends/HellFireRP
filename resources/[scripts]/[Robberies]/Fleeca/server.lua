...
function MFF:RewardPlayer(data,id)
  ...
  local dinero = math.random(1000,2000)
  ...
  if id then
    ...
    TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Robaste: $'..dinero, length = 2500, style = { ['background-color'] = '#B300BF', ['color'] = '#ffffff' } })	
  end
end
...
function MFF:GetIdCount(source)  
  local xPlayer = ESX.GetPlayerFromId(source)
  while not xPlayer do Citizen.Wait(0); xPlayer = ESX.GetPlayerFromId(source); end
  if xPlayer.getInventoryItem('bankidcard').count >= 1 then
