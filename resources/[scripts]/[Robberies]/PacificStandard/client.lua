...
UTK = {
    ...
    enablextras = false, -- enable gold and diamond looting (yay!) [needs utk_ornateprops]
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports["mythic_notify"]:DoHudText("success", "Encuentra la IP...")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports["mythic_notify"]:DoHudText("success", "Encuentra la contraseña...")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports["mythic_notify"]:DoHudText("success", "Ejecuta BruteForce.exe")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:DoHudText("error", "Tienes 90 segundos antes que se active el gas.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports["mythic_notify"]:DoHudText("success", "Robaste "..text)
        end)
    else
        exports['mythic_notify']:DoHudText("error", "Tienes 90 segundos antes que entre el gas.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["mythic_notify"]:DoHudText("error", "La boveda se sellará en 60 segundos")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["mythic_notify"]:DoHudText("error", "BOVEDA CERRANDOSE!")
...
AddEventHandler("utk_oh:policenotify", function()
    TriggerServerEvent('MF_Trackables:Notify',"Alarma activada en Pacific Standard",GetEntityCoords(GetPlayerPed(-1)),"police")
end)
