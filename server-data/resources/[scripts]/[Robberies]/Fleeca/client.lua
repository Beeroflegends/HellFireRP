  ...
  if closest.val == "LockpickDoor" then
   ESX.TriggerServerCallback('MF_Fleeca:GetLockpickCount', function(count)
      if count and count > 0 then
        TriggerServerEvent('MF_Trackables:Notify','Alarma activada en Fleeca.',GetEntityCoords(GetPlayerPed(-1)),'police')
        TriggerEvent('lockpicking:StartMinigame')
      else
        exports['mythic_notify']:DoHudText('error', 'No tienes ganzuas')
  ...
  elseif closest.val == "LootVault" then
    ESX.TriggerServerCallback('MF_Fleeca:GetOxyCount', function(count)
      if count and count > 0 then
        ...
        exports['mythic_notify']:DoHudText('error', 'No tienes cortador')
        self.Interacting = false
      end
    end)
  end
...
function MFF:LootHandler(closest,idcard)
  ESX.TriggerServerCallback('MF_Fleeca:TryLoot',function(isLooted)
    if not isLooted then
      ...
      if idcard then
        exports['progressBars']:startUI(self.InteractTimer * 4000, "Buscando")
        TaskStartScenarioInPlace(plyPed, "PROP_HUMAN_BUM_BIN", 0, true)
      else
        exports['progressBars']:startUI(self.InteractTimer * 4000, "Cortando")
        TaskStartScenarioInPlace(plyPed, "WORLD_HUMAN_WELDING", 0, true)
      end
      Wait(self.InteractTimer * 4000)
...
function MFF:HandleVaultDoor(closest)
  ESX.TriggerServerCallback('MF_Fleeca:GetIDCount', function(count)
    if count and count > 0 then
      ...
      FreezeEntityPosition(plyPed,true)
      exports['progressBars']:startUI(self.InteractTimer * 1000, "Solicitando Acceso")
...
--  ESX.ShowNotification('Somebody is robbing fleeca bank!')
  ...
  Citizen.CreateThread(function(...)
    ...
    AddTextComponentString      ("Robo en Proceso")
...
AddEventHandler('lockpicking:MinigameComplete', function(result) MFF:LockpickComplete(result); end)
