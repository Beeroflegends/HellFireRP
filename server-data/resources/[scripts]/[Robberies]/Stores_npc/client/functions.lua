...
Initial = function()
	ESX.TriggerServerCallback("storerobbery:createClerks", function(response)
...
CreateClerks = function()
	...
	ESX.TriggerServerCallback("storerobbery:updateClerks", function(updated)
		if updated then
			-- correctly updated.
		end
	end, networkedClerks)
...
RobPed = function(pedEntity)
	...
	if math.random(3) == 2 then
		...
		GiveWeaponToPed(pedEntity, GetHashKey("WEAPON_PUMPSHOTGUN"), 12, false, true)
...
DrawTimerBar = function(percent)
	...
	local X, Y, W, H = 1.015 - correction, 1.475 - correction, percent * 0.00085, 0.0125
