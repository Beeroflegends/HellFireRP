          ...
          if closestDist < self.InteractDist then
            if not safe then              
              ...
              if (not self.Looted or (self.Looted and not self.Looted[key])) and lootRemains then
                Utils:DrawText3D(val.Pos.x,val.Pos.y,val.Pos.z, "Presiona [~r~E~s~] para quebrar el vidrio.")
                if Utils:GetKeyPressed("E") then
                  self:Interact(key,val, plyPed,false)
                end
              end
            elseif not self.SafeUsed then          
              Utils:DrawText3D(self.SafePos.x,self.SafePos.y,self.SafePos.z, "Presiona [~r~E~s~] para forzar la caja.")
...
function MFV:Interact(key,val, plyPed, safe)
  if not safe then
   ...
   --    TriggerServerEvent('MF_Vangelico:NotifyCops')
		 TriggerServerEvent('MF_Trackables:Notify',"Alarma activada en Joyeria", GetEntityCoords(GetPlayerPed(-1)),'police')
      end

      self.DoingAction = false
    elseif not plyWeapon then
      ESX.ShowNotification("Necesitas algo para quebrar el vidrio.")
    elseif plySkin["bags_2"] == 0 and plySkin["bags_1"] == 0 then
      ESX.ShowNotification("Como piensas cargar todo esto?.")
    elseif matching then
      ESX.ShowNotification("No puedes quebrar el vidrio con esto.")      
    end
  else 
    self.SafeUsed = true
    ESX.TriggerServerCallback('MF_Vangelico:GetSafeState', function(canUse)
      if canUse then
        ...
        ESX.ShowNotification("Alguien ya abrió esta caja.")
      end
    end)
  end
end
...
function MFV:DoNotifyPolice(...)
  ...
  if plyData.job.name == self.PoliceJobName then
    ESX.ShowNotification('Alguien esta robando la Joyeria.')
  end
end
