...
--		{ label = "Sell", action = "PawnShop_Sell_Menu" },
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "weapon_shop_main_menu",
		{
			title    = "Store",
			align    = "bottom",
			elements = elements
...
function PawnShopBuyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "weapon_shop_buy_menu",
		{
			title    = "Illegal Store",
			align    = "right",
			elements = elements
...
function OpenBuyDialogMenu(itemName, BuyPrice)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'weapon_shop_amount_to_buy_dialog', {
		title = "Amount to Buy?"
	}, function(data, menu)
		menu.close()
		amountToBuy = tonumber(data.value)
		totalBuyPrice = (BuyPrice * amountToBuy)
		TriggerServerEvent("weapon_shop:BuyItem",amountToBuy,totalBuyPrice,itemName)
	end,
	function(data, menu)
		menu.close()	
	end)
...
function PawnShopSellMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "weapon_shop_sell_menu",
		{
			title    = "What you wanna sell?",
			align    = "bottom",
			elements = elements
		},
	function(data, menu)
...
function OpenSellDialogMenu(itemName, SellPrice)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'weapon_shop_amount_to_sell_dialog', {
		title = "Cantidad a vender?"
	}, function(data, menu)
		menu.close()
		amountToSell = tonumber(data.value)
		totalSellPrice = (SellPrice * amountToSell)
		TriggerServerEvent("weapon_shop:SellItem",amountToSell,totalSellPrice,itemName)
	end,
	function(data, menu)
		menu.close()	
	end)
