RegisterNetEvent("cajafuerte:start")
AddEventHandler("cajafuerte:start", function()
	TriggerEvent("cajafuerte:loop",10)
end)

cracking = false
RegisterNetEvent("cajafuerte:loop")
AddEventHandler("cajafuerte:loop", function()

TriggerServerEvent('MF_Trackables:Notify','Alarm triggered on Store',GetEntityCoords(GetPlayerPed(-1)),'police')
	...
	while cracking do
		...
		DisplayHelpText("Controls [E] left | [F] right | [H] confim")
	...
	if curLock > difficulty then
		TriggerServerEvent("cajafuerte:dinero")
	end
	resetAnim()
