...
if Config.SqlKeybinding then
  MySQL.ready(function()
	...
	AddEventHandler("dp:ServerKeybindCreate", function()
		local src = source local srcid = GetPlayerIdentifier(source)
		MySQL.Async.execute('INSERT INTO dpkeybinds (`id`, `keybind1`, `emote1`, `keybind2`, `emote2`, `keybind3`, `emote3`, `keybind4`, `emote4`, `keybind5`, `emote5`, `keybind6`, `emote6`, `keybind7`, `emote7`, `keybind8`, `emote8`, `keybind9`, `emote9`) VALUES (@id, @keybind1, @emote1, @keybind2, @emote2, @keybind3, @emote3, @keybind4, @emote4, @keybind5, @emote5, @keybind6, @emote6, @keybind7, @emote7, @keybind8, @emote8, @keybind9, @emote9);',
		{id = srcid, keybind1 = "num4", emote1 = "", keybind2 = "num5", emote2 = "", keybind3 = "num6", emote3 = "", keybind4 = "num7", emote4 = "", keybind5 = "num8", emote5 = "", keybind6 = "num9", emote6 = "", keybind7 = "z", emote7 = "", keybind8 = "x", emote8 = "", keybind9 = "g", emote9 = ""}, function(created) print("[dp] ^2"..GetPlayerName(src).."^7 got created!") TriggerClientEvent("dp:ClientKeybindGet", src, "num4", "", "num5", "", "num6", "", "num7", "", "num8", "", "num8", "", "z", "", "x", "", "g", "") end)
	end)

	RegisterServerEvent("dp:ServerKeybindGrab")
	AddEventHandler("dp:ServerKeybindGrab", function()
		local src = source local srcid = GetPlayerIdentifier(source)
		MySQL.Async.fetchAll('SELECT keybind1, emote1, keybind2, emote2, keybind3, emote3, keybind4, emote4, keybind5, emote5, keybind6, emote6, keybind7, emote7, keybind8, emote8, keybind9, emote9 FROM `dpkeybinds` WHERE `id` = @id',
		{['@id'] = srcid}, function(kb)
			if kb[1].keybind1 ~= nil then
				TriggerClientEvent("dp:ClientKeybindGet", src, kb[1].keybind1, kb[1].emote1, kb[1].keybind2, kb[1].emote2, kb[1].keybind3, kb[1].emote3, kb[1].keybind4, kb[1].emote4, kb[1].keybind5, kb[1].emote5, kb[1].keybind6, kb[1].emote6, kb[1].keybind7, kb[1].emote7, kb[1].keybind8, kb[1].emote8, kb[1].keybind9, kb[1].emote9)
			else
				TriggerClientEvent("dp:ClientKeybindGet", src, "num4", "", "num5", "", "num6", "", "num7", "", "num8", "", "num8", "", "z", "", "x", "", "g", "")
			end
		end)
	end)
	...
	AddEventHandler("dp:ServerKeybindUpdate", function(key, emote)
		...
		elseif chosenk == "keybind6" then
			...
			elseif chosenk == "keybind7" then
			MySQL.Async.execute("UPDATE dpkeybinds SET emote7=@emote WHERE id=@id", {id = myid, emote = emote}, function() TriggerClientEvent("dp:ClientKeybindGetOne", src, key, emote) end)
			elseif chosenk == "keybind8" then
			MySQL.Async.execute("UPDATE dpkeybinds SET emote8=@emote WHERE id=@id", {id = myid, emote = emote}, function() TriggerClientEvent("dp:ClientKeybindGetOne", src, key, emote) end)
			elseif chosenk == "keybind9" then
			MySQL.Async.execute("UPDATE dpkeybinds SET emote9=@emote WHERE id=@id", {id = myid, emote = emote}, function() TriggerClientEvent("dp:ClientKeybindGetOne", src, key, emote) end)
		end
	end)
  end)
else
