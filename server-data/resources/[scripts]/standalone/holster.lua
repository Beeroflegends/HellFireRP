...
--		ESX.ShowNotification('No Holster')
		 exports['mythic_notify']:DoHudText('error', 'No holster')
	-- Chain 8
    elseif args[1] == '1' then
		holster = 1
--		ESX.ShowNotification('Holster Chain 8')
		exports['mythic_notify']:DoHudText('success', 'Holster on')
   
    else
 --       ESX.ShowNotification('Incorrect use: /holster 0,1,2,3')
		 exports['mythic_notify']:DoHudText('error', 'Wrong use | /holster 1')
    end
end, false)

--for different holsters for males and females
RegisterCommand("genre", function(source, args, raw)
    if args[1] == 'm' then
		sex = 0
...
--		ESX.ShowNotification('You need to be at a trunk to draw that weapon!')
		exports['mythic_notify']:DoHudText('error', 'Get close to a vehicle trunk')
	else
		blocked = true
		removeWeaponOnBack()
		startAnim("mini@repair", "fixing_a_ped")
...
--		ESX.ShowNotification('You need to be at a trunk to put away your weapon!')
		exports['mythic_notify']:DoHudText('error', 'Rack the weapon back to the vehicle')
	end
	racking = false
end
