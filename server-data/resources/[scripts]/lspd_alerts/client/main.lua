	...
	while true do
		...
		if IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and Config.GunshotAlert then
			...
			if (isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted then
				...
				}, streetName, zoneNameFull, playerGender)
				Citizen.Wait(30000)
			end
			
		elseif isDead or GetEntityHealth(playerPed) <= 0 and isPlayerWhitelisted then
		Citizen.Wait(3000)
			...
			if (isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted then
				...
				TriggerServerEvent('dovux:herido', {
					x = ESX.Math.Round(playerCoords.x, 1),
					y = ESX.Math.Round(playerCoords.y, 1),
					z = ESX.Math.Round(playerCoords.z, 1)
		...
		elseif (IsPedTryingToEnterALockedVehicle(playerPed) or IsPedJacking(playerPed)) and Config.CarJackingAlert then
				...
				ESX.TriggerServerCallback('esx_outlawalert:isVehicleOwner', function(owner)
					if not owner then
						...
						TriggerServerEvent('esx_outlawalert:carJackInProgress', {
							...
							y = ESX.Math.Round(playerCoords.y, 1),
							z = ESX.Math.Round(playerCoords.z, 1)
						}, streetName, vehicleLabel, primary, plate, zoneNameFull)
						
						Citizen.Wait(30000) -- No mandar alertas tan seguido
					end
				end, plate)
			end
		end
...
RegisterNetEvent('dovux:blipherido')
AddEventHandler('dovux:blipherido', function(targetCoords)
	if isPlayerWhitelisted then
		if Config.LSPD then
			local alpha = 250
			local LSPDBlip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, Config.LSPDRadius)

			SetBlipHighDetail(LSPDBlip, true)
			SetBlipColour(LSPDBlip, 1)
			SetBlipAlpha(LSPDBlip, alpha)
