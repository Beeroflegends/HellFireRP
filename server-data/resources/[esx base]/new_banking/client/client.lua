...
local atms = {
  ...
  {name="ATM", id=277, x=89.57, y=2.42, z=68.31},
   {name="ATM", id=277, x=315.17, y=-593.75, z=43.28},
   {name="ATM", id=277, x=940.91, y=-987.56, z=39.86},
    {name="ATM", id=277, x=934.54, y=31.45, z=80.26},
	 {name="ATM", id=277, x=942.71, y=44.63, z=80.26},
}
--================================================================================================
...
if bankMenu then
	...
	if nearBank() or nearATM() then
			DisplayHelpText("Press ~INPUT_PICKUP~ to open menu ~b~")

		if IsControlJustPressed(1, 38) then
		exports['progressBars']:startUI(2500, 'Giving card')  
			playAnim('mp_common', 'givetake1_a', 2500)
			SetNuiFocus(true, false)
			inMenu = false
			Citizen.Wait(2500)
			exports['mythic_notify']:DoHudText('inform', 'Welcome to Fleeca!', 2500, { ['background-color'] = '#31a4eb', ['color'] = '#ffffff' })
...
RegisterNUICallback('deposit', function(data)
	TriggerServerEvent('bank:deposito', tonumber(data.amount))
	TriggerServerEvent('bank:balance')
end)
...
RegisterNUICallback('withdrawl', function(data)
	TriggerServerEvent('bank:retirar', tonumber(data.amountw))
	TriggerServerEvent('bank:balance')
end)
...
RegisterNUICallback('NUIFocusOff', function()
	...
	exports['progressBars']:startUI(2500, 'Taking card')
			playAnim('mp_common', 'givetake1_a', 2500)
			SetNuiFocus(true, false)
			inMenu = false
			Citizen.Wait(2500)
			exports['mythic_notify']:DoHudText('inform', 'Have a nice day!', 2500, { ['background-color'] = '#31a4eb', ['color'] = '#ffffff' })
			Citizen.Wait(100)
			SetNuiFocus(false, false)
