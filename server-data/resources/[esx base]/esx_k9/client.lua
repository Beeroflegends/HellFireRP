...
AddEventHandler('esx_policedog:hasDrugs', function(hadIt)
   ...
   --     ESX.ShowNotification(Strings['drugs_found'])
		exports['mythic_notify']:DoHudText('inform', 'The dog found drugs', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
        loadDict('missfra0_chop_find')
        TaskPlayAnim(policeDog, 'missfra0_chop_find', 'chop_bark_at_ballas', 8.0, -8, -1, 0, 0, false, false, false)
    else
	exports['mythic_notify']:DoHudText('inform', 'No drugs found', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
...
mainMenu = function()
            ...
            function(data, menu)
                if data.current.value == 'take_out_remove' then
                    if not DoesEntityExist(policeDog) then
                        ...
                        while not HasModelLoaded(Config.Model) do Wait(500) end
                  ...
                  --      ESX.ShowNotification(Strings['spawned_dog'])
						exports['mythic_notify']:DoHudText('inform', 'K9 unit in service', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
                    else
					exports['mythic_notify']:DoHudText('inform', 'K9 unit out of service', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
                ...
                elseif data.current.value == 'attack_closest' then
                    if DoesEntityExist(policeDog) then
                        if not IsPedDeadOrDying(policeDog) then
                            ...
                            exports['mythic_notify']:DoHudText('inform', 'The dog is dead', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
                        end
                    else
                       exports['mythic_notify']:DoHudText('inform', 'You dont have dogs', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
                    end
                elseif data.current.value == 'sit_stand' then
                    if DoesEntityExist(policeDog) then
                      ...
                      --      ESX.ShowNotification(Strings['dog_too_far'])
							exports['mythic_notify']:DoHudText('inform', 'Dog is too far', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
                        end
                    else
                ...
                elseif data.current.value == 'search_drugs' then
                    if DoesEntityExist(policeDog) then
                       ...
                       --     ESX.ShowNotification(Strings['dog_dead'])
							exports['mythic_notify']:DoHudText('inform', 'Your dog is dead', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
                        end
                    else
					exports['mythic_notify']:DoHudText('inform', 'You dont have dogs', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
                 --       ESX.ShowNotification(Strings['no_dog'])
 ...
 --       ESX.ShowNotification(Strings['not_police'])
		exports['mythic_notify']:DoHudText('inform', 'You are not a cop', 2500, { ['background-color'] = '#CE0000', ['color'] = '#ffffff' })
    end
end
