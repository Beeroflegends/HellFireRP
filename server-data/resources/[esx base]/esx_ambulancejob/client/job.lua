...
AddEventHandler('commando:levantarherido', function()
	...
	if closestPlayer == -1 or closestDistance > 1.1 then
				exports['mythic_notify']:DoHudText('error', 'no_players',3500)
				else	
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			...
			if closestPlayer ~= -1 and closestDistance <= 2.0 then
					...
					elseif action == 'factura' then 
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
							...
							if amount == nil or amount < 0 then
								exports['mythic_notify']:DoHudText('error',U('invalid_amount'),7000)				
								--ESX.ShowNotification(_U('invalid_amount'))
							else
								local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
								if closestPlayer == -1 or closestDistance > 3.0 then
								exports['mythic_notify']:DoHudText('error',_U('no_players'),7000)					
				...
				exports['mythic_notify']:DoHudText('error', _U('no_players_nearby'),7000)
				end
		...
		elseif data.current.value == 'vehicle_interaction' then
			...
			ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
				local target, distance = ESX.Game.GetClosestPlayer()
						   ...
						   exports['mythic_notify']:DoHudText('success',_U('revive_complete_award', GetPlayerName(closestPlayer), Config.ReviveReward),4500)
							--ESX.ShowNotification(_U('revive_complete_award', GetPlayerName(closestPlayer), Config.ReviveReward))
									else									
							 exports['mythic_notify']:DoHudText('success',_U('revive_complete', GetPlayerName(closestPlayer)),5500)				
							--ESX.ShowNotification(_U('revive_complete', GetPlayerName(closestPlayer)))
									end															
								else
								exports['mythic_notify']:DoHudText('nohayjugadorescerca', ' player not unconscious ',2500)
									--ESX.ShowNotification(_U('player_not_unconscious'))
								end						
							else
							 exports['mythic_notify']:DoHudText('error', _U('not_enough_medikit') ,3000)	
					    ...
					    TriggerServerEvent('ems:nuevobb',target_id, playerheading, playerCoords, playerlocation)
						   ...
						   exports['mythic_notify']:DoHudText('success',_U('revive_complete_award', GetPlayerName(closestPlayer), Config.ReviveReward),4500)
							 ...
							 exports['mythic_notify']:DoHudText('success',_U('revive_complete', GetPlayerName(closestPlayer)),5500)
								...
								exports['mythic_notify']:DoHudText('error', ' player not unconscious ',2500)
								end						
							else
							 exports['mythic_notify']:DoHudText('error', _U('not_enough_medikit') ,3000)	
							end
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									local playerPed = PlayerPedId()
									IsBusy = true
										exports['mythic_notify']:DoHudText('success',_U('heal_inprogress'),8000)							
								...
								TaskStartScenarioInPlace(playerPed, 'CODE_HUMAN_MEDIC_TEND_TO_DEAD', 0, true)
								 exports['progressBars']:startUI(5000, "Aplicande Vendajes")
									...
									TriggerServerEvent('esx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')															
			 						exports['mythic_notify']:DoHudText('cajaverde',_U('heal_complete', GetPlayerName(closestPlayer)),5500)						
									--ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)))
									IsBusy = false

								else
								 exports['mythic_notify']:DoHudText('error',_U('player_not_conscious'),7000)					
								--ESX.ShowNotification(_U('player_not_conscious'))
								end
							else
								exports['mythic_notify']:DoHudText('error',_U('not_enough_bandage'),7000)
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									IsBusy = true					
							exports['mythic_notify']:DoHudText('success',_U('heal_inprogresse'),8000)
									...
									TriggerServerEvent('esx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
								          exports['mythic_notify']:DoHudText('success',_U('heal_complete', GetPlayerName(closestPlayer)),8000)								
									--ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)))
									IsBusy = false
								else
							exports['mythic_notify']:DoHudText('error',_U('player_not_conscious'),8000)
									--ESX.ShowNotification(_U('player_not_conscious'))
								end
							else
							exports['mythic_notify']:DoHudText('error',_U('not_enough_medikit'),8000)
								--ESX.ShowNotification(_U('not_enough_medikit'))
							end
						end, 'medikit')
		...
		for hospitalNum,hospital in pairs(Config.Hospitals) do
			...
			for k,v in ipairs(hospital.AmbulanceActions) do
         	...
         	if GetDistanceBetweenCoords(coords, 313.69, -590.63,28.95, true) < 3.0 then
                DrawText3Ds(vector3(313.69, -590.63,28.95), "Press [E] to open menu", 0.37)
			end	
			...
			for k,v in ipairs(hospital.Pharmacies) do
         	...
         	if GetDistanceBetweenCoords(coords, 355.30, -577.43,  28.85, true) < 2.0 then
                DrawText3Ds(vector3(355.30, -577.43,  28.85), "Press [E] to open menu", 0.37)
			end	
...
exports['mythic_notify']:DoHudText('success', _U('garage_released'),7500)
		--ESX.ShowNotification(_U('garage_released'))
								end)
							end
						else
	exports['mythic_notify']:DoHudText('error', _U('garage_notavailable'),7500)
		...
		exports['mythic_notify']:DoHudText('error', _U('garage_empty'),7500)
...
exports['mythic_notify']:DoHudText('error', _U('garage_store_nearby'),7500)
	...
	ESX.TriggerServerCallback('esx_ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			...
			IsBusy = false
			exports['mythic_notify']:DoHudText('success', _U('garage_has_stored'),7500)		
			--ESX.ShowNotification(_U('garage_has_stored'))
		else
			exports['mythic_notify']:DoHudText('error', _U('garage_has_notstored'),7500)	
...
exports['mythic_notify']:DoHudText('error', _U('garage_blocked'),7500)
...
function OpenHelicopterSpawnerMenu(hospital, partNum)
	...
	exports['mythic_notify']:DoHudText('error', _U('helicopter_notauthorized'),7500)
...
function OpenShopMenu(elements, restoreCoords, shopCoords)
	...
	exports['mythic_notify']:DoHudText('success', _U('vehicleshop_bought', data.current.name, ESX.Math.GroupDigits(data.current.price)),7500)
		...
		exports['mythic_notify']:DoHudText('error', _U('vehicleshop_money'),7500)
...
function WarpPedInClosestVehicle(ped)
	...
	if distance ~= -1 and distance <= 5.0 then
		...
		exports['mythic_notify']:DoHudText('error', _U('no_vehicles'),7500)
...
AddEventHandler('esx_ambulancejob:heal', function(healType, quiet)
	...
	if not quiet then
	exports['mythic_notify']:DoHudText('success', _U('healed'),7500)
...
RegisterCommand('dragout',function()
		local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
       ...
       if closestPlayer ~= -1 and closestDistance <= 2.0 then 
		TriggerServerEvent('esx_sem:OutVehicle', GetPlayerServerId(closestPlayer))			
			...
			exports['mythic_notify']:DoHudText('error', 'No players nearby',2200)
             end
	
end, false)	

RegisterCommand('dragin',function()
		local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
...
TriggerEvent('chat:addSuggestion', '/takeout', 'Drag out a player from vehicle', {})
