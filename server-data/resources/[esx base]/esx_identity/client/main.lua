...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[ERROR]', '^1You must create a character in order to play' } })
	end
end)
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Your character name is too long or too short to be acceptable'
	end
	...
	if count ~= nameLength then
		return 'You character name contains an invalid symbol'
	end
	...
	if spacesInName > 2 then
		return 'Your name contains 2 or more spaces'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'Names starts with capital letter'
	end
