...
function OpenPropertyMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		ESX.TriggerServerCallback('pxrp_vip:getVIPStatus', function(isVIP)
			...
			exports['mythic_notify']:DoHudText('error', "You're not a VIP")
					end
...
function OpenRoomMenu(property, owner)
	...
	if CurrentPropertyOwner == owner then
		table.insert(elements, {label = "Inventory", value = "property_inventory"})
		table.insert(elements, {label = "Dressing", value = "player_dressing"})
	end
